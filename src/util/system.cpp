...
const char * const BITCOIN_CONF_FILENAME = "reditcoin.conf";
...
#else
    const char* pszModule = "reditcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Reditcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Reditcoin";
#else
    // Unix
    return pathRet / ".reditcoin";
#endif
#endif
}
