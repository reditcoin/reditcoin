...
#if defined(NDEBUG)
# error "Reditcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Reditcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s RDT additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // premine 30m at block 10th
    if (nHeight == 10)
	nSubsidy = 30000000 * COIN;

    // premine 60m at block 12th
    if (nHeight == 12)
...
void ThreadScriptCheck() {
    RenameThread("reditcoin-scriptch");
