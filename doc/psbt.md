# PSBT Howto for Reditcoin Core

Since Reditcoin Core 0.17, an RPC interface exists for Partially Signed Reditcoin
...
PSBT is an interchange format for Reditcoin transactions that are not fully signed
...
#### Multisig with multiple Reditcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Reditcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `reditcoin-cli` is used.
